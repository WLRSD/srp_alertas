...
for a in range(len(dfForm['id'])):
 #   dados_atual = resultados[a]  # 'resultados[a]' é um DataFrame
    ...
    if data_envio is not None:  # Verifica se 'dt_alerta' já não é None
        ...
        if dfForm['produto'][a] == 'Datavalid':
            url = "https://serprogovbr.webhook.office.com/webhookb2/2157f8e6-a9dc-4606-9114-78c95956f4cb@c2db1c4d-29e6-4330-85e3-fb54b7696668/IncomingWebhook/f0469391055443f29b9812bc7bb634ff/ab7201cc-0b6c-4366-ae7d-b30e8b6c6109/V2MUH7SUh2sloSAOlVLbzrV0w0Kc5tf_lgk3TMDmCnmJY1"
        elif dfForm['produto'][a] == 'Serpro MultiCloud':
            url ="https://serprogovbr.webhook.office.com/webhookb2/2157f8e6-a9dc-4606-9114-78c95956f4cb@c2db1c4d-29e6-4330-85e3-fb54b7696668/IncomingWebhook/dab0ee9062db4add9042849cf0a7c8ec/ab7201cc-0b6c-4366-ae7d-b30e8b6c6109/V29oQPkMvC-q3beFWOCmrQelq9PwqR90XMnCTPy8Boqh81"
        else:
        ...
        if tipo == "Valor acima":  
            if valor_value < total_value :       
                if dfForm['produto'][a]:
                    msg = {
                        "text": f"Mensagem enviada via Python após verificação ({dfForm['tipo_alerta'].values[a]}): O valor do faturamento {dfForm['assunto_alerta'].str[18:-1][a]} de {dfFat['cd_ano'].values[a]} do produto  *{dfForm['produto'][a]}* ultrapassou o valor de {valor_monetario}, hoje está em {total_monetario}"
                    }
                else :
                       msg = {
        ...
        elif tipo == "Valor abaixo":  
            if valor_value > total_value:       
                if dfForm['produto'][a]:
                    msg = {
                        "text": f"Mensagem enviada via Python após verificação ({dfForm['tipo_alerta'].values[a]}): O valor do faturamento {dfForm['assunto_alerta'].str[18:-1][a]} de {dfFat['cd_ano'].values[a]} do produto  *{dfForm['produto'][a]}* ficou abaixo de {valor_monetario}, hoje está em {total_monetario}"
                    }
                else :
                       msg = {
                        "text": f"Mensagem enviada via Python após verificação ({dfForm['tipo_alerta'].values[a]}): O valor do faturamento {dfForm['assunto_alerta'].str[18:-1][a]} de {dfFat['cd_ano'].values[a]} ficou abaixo de {valor_monetario}, hoje está em {total_monetario}"
                    } 
